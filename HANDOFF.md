...
skips GPS), `/?lang=en|hi|kn` (force the UI language).
...
chat toggle uses). Keep new UI strings in all three languages.
...
- `services/plain_language.py` — non-technical advice in EN/HI/KN.
...
- Fonts (all self-hosted via `@fontsource-variable/*`, offline-safe):
  ...
  **Noto Serif Devanagari** for hi headings. Nirmala UI remains the
...
| Kannada question answered in English | The UI was forcing its language selection over server-side detection. Language is now auto-detected unless the user explicitly selects a language. |
...
3. Ask in Kannada (Ask ORCA tab, scenario 2) — Kannada in, Kannada out, 70/100 HIGH.
