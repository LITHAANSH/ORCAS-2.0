...
CASES = [
    ("Scenario 2 (KILLER DEMO) — Kannada, Mumbai 6 AM",
     "ನಾಳೆ ಬೆಳಿಗ್ಗೆ 6 ಗಂಟೆಗೆ ಮುಂಬೈ ಹತ್ತಿರ ಮೀನುಗಾರಿಕೆಗೆ ಹೋಗಬಹುದೇ?", "s1"),
    ("Follow-up — what about 12 PM? (context retained)",
     "ಮಧ್ಯಾಹ್ನ 12 ಗಂಟೆಗೆ ಹೇಗಿರುತ್ತದೆ?", "s1"),
...
def main() -> int:
    ...
    r1 = planner.handle(ChatRequest(
        message="ನಾಳೆ ಬೆಳಿಗ್ಗೆ 6 ಗಂಟೆಗೆ ಮುಂಬೈ ಹತ್ತಿರ ಮೀನುಗಾರಿಕೆಗೆ ಹೋಗಬಹುದೇ?", session_id="a"))
    if r1.language != "kn":
        failures.append(f"Mumbai query language detected as {r1.language}, expected kn")
    ...
    r2 = planner.handle(ChatRequest(message="ಮಧ್ಯಾಹ್ನ 12 ಗಂಟೆಗೆ ಹೇಗಿರುತ್ತದೆ?", session_id="a"))
